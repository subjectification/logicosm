...
def createAcc():
    n = raw_input("Please enter a name for the new account:")
    if n.lower() in (name.lower() for name in accounts):
        refresh(0, "An account already exists with that name!")
    else:
        #pw = hash(raw_input("Please enter a password for this account:"))
        global current_account
    ...
    if len(accounts) > 0:
        ...
        if n == "n":
            ...
            if int(n)-1 <= len(accounts):
                #pw = hash(raw_input("Please enter your password:"))
                #if pw == truepw:
                load_account(name)
                #else:
...
def interface(state):
    if state == 0:          # account not selected
        ...
        if inp == '1':
            ...
            pw = raw_input("Please enter the password to access this feature:")
            if pw != password:
                refresh(0.1, "Sorry! That password was not correct!")
            else:
                num = raw_input("How much would you like to add?\n")
                if num.isdigit():
        ...
        if t == 'q':
            ...
            refresh(2, "That isn't an raw_input I recognise :(")
                    
    elif state == 3:       # Exchange market
        printInvestments()
        print_balance()
        print_portfolio()
        inp = raw_input("Press 'b' to buy, 's' to sell, 'h' to check the history of prices, or 'q' to return to the main screen\n")
        if inp == 'b':
        ...
        printInvestments()
        print_balance()
        inp = raw_input("What would you like to buy? Or press 'e' to return to the main exchange market, or 'q' to return to the main menu\n")
        if inp == 'e':
            refresh(3, "Exchange market")
        elif inp == 'q':
            refresh(0.1, "Welcome!")
        elif inp.isdigit() and int(inp) < len(investments):
            n = raw_input("How much " + investments[int(imp)] + " would you like to buy?\n")
            current_account.buy(inp, int(n))
        elif inp.upper() in investments:
            n = raw_input("How much " + inp.upper() + " would you like to buy?\n")
        ...
        printInvestments()
        print_portfolio()
        inp = raw_input("What would you like to sell? Or press 'e' to return to the main exchange market, or 'q' to return to the main menu\n")
        if inp == 'e':
            refresh(3, "Exchange market")
        elif inp == 'q':
            refresh(0.1, "Welcome!")
        elif inp.isdigit() and int(inp) <= len(investments):
            n = raw_input("How much " + investments[int(inp)] + " would you like to sell?\n")
            current_account.sell(inp, int(n))
        elif inp.upper() in investments:
            n = raw_input("How much " + inp.upper() + " would you like to sell?\n")
            current_account.sell(inp.upper(), int(n))
        else:
            refresh(3.2, "I don't know what that is :(")
    elif state == 3.3:      #price history
        l = raw_input("Select a number of days to look back on, press 'e' to go back to the exchange market, or press 'q' to go back to the main menu\n")
        if l == 'q':
            refresh(0.1, "Welcome!")
        elif l == 'e':
            refresh(3, "Exchange market")
        ...
        inp = raw_input("Press 'e' to return to the exchange market, 'q' to return to the main menu, or 'h' to check the history again:")
        if inp == 'e':
            refresh(3, "Exchange market")
        elif inp == 'q':
            refresh(0.1, "Welcome!")
        ...
        next = raw_input("Press any key to return to the main menu.")
        refresh(0.1, "Welcome!")
...
def try_again(selection, true_ans):
    global combo
    combo = 0
    next = raw_input("The correct answer is " + str(true_ans) + ". Press any key to try again, or 'q' to return to the main menu\n")
    if next == 'q':
        refresh(0.1, "Welcome!")
    else:
        journey(selection)
...
def journey(selection):
    while True:
        ...
        if selection == '1':    # Addition
            ...
            fst = random.randint(0,9)
            snd = random.randint(0,9)
            ans = raw_input(str(fst) + " x " + str(snd) + " = ")
            while not ans.isdigit():
                clear()
                print("Please enter a number:")
...
def success(selection):
    ...
    if reward > 0:
        ...
        save()
        next = raw_input("Correct! You have been given $" + colorama.Fore.GREEN + str(reward) + colorama.Fore.RESET + beep + " for your hard work. Your balance is now $" + colorama.Fore.GREEN + str(current_account.balance) + colorama.Fore.RESET + ". Press enter for another question, or 'q' to return to the main menu\n")
        if next == 'q':
            refresh(0.1, "Welcome!")
        else:
            journey(selection)            
    else:
        next = raw_input("Correct! Press enter for another question, or 'q' to return to the main menu\n")
        if next == 'q':
            refresh(0.1, "Welcome!")
        else:
            journey(selection) 
